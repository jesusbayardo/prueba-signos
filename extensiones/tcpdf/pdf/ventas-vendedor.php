...
class imprimir
{
    ...
    public function traerImpresionFactura()
    {
        ...
        foreach ($respuesta as $key1 => $value1) {
            ...
            foreach ($respuesta as $key1 => $value1) {
                if ($value1["estado"]==0) {
                    if ($_GET["vdtegdo"] == $value1["id_vendedor"]) {
                        $valorVentaa = ($valorVentaa + $value1["total_pagar"]);
                    }
    
                    if ($_GET["vdtegdo"] == $value1["id_vendedor"] && $value1["metodo_pago"]=="Credito") {
                         $valorCredito = ( $valorCredito + $value1["total_pagar"]);
                    }
    
                    if ($_GET["vdtegdo"] == $value1["id_vendedor"] && $value1["metodo_pago"]=="Efectivo") {
	...
	<table>
        ...
        <br><br>
			...
			<td style="background-color:white; width:280px">
                ...
                <div style="font-size:8px; text-align:center; line-height:10px;">
...
EOF;
        ...
        foreach ($respuesta as $key1 => $value1) {
            ...
            $respuestaClientes = ControladorClientes::ctrMostrarClientes($item, $id_cliente);
            if ($respuestaClientes == false) {
                $nombresCliente = "CLIENTE ELIMINADO";
            } else {
                $nombresCliente = $respuestaClientes["nombre"];
            }
            ...
            if ($respuestaUsuarios != false) {
                if ($value1["estado"]==0) {
                if ($_GET["vdtegdo"] == $respuestaUsuarios["id"]) {
                    $sum = $sum + 1;
                    $bloque4 = <<<EOF
        ...
        $pdf->Output('reporte-vendedor.pdf');
    }
}
